...
function Set-UserAdmin {
    ...
    Do {
        $setAdmin = Read-Host "Veuillez-vous activer l'UAC -> User Access Control' ? [o/n]"
        if (($setAdmin -ine "O") -or ($setAdmin -ine "n")) {
            Write-Host "Choisissez O pour Oui ou N pour Non"
        }
    } while (($setAdmin -ine "o") -or ($setAdmin -ine "n"))
